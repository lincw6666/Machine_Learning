...
int main(int argc, char** argv) 
{
	...
	struct flower train[TRAIN_DATA_SIZE];
	struct flower flowers[SUB_TRAIN];
	struct node* root[ROOT_NUM];
	...
	for (int test_data_start = 0; test_data_start < DATA_SIZE; test_data_start += 30) {
		...
		for (int i = 0, now = 0; i < DATA_SIZE; i++) {
			if (i < test_data_start || i >= test_data_start+30) {
				for (int j = 0; j < FEATURE_NUM; j++) 
					train[now].feature[j] = data[i].feature[j];
				train[now].class = data[i].class;
				now++;
			}
		}

		for (int i = 0, cnt = 0; i < ROOT_NUM-1; i++, cnt += TRAIN_DATA_SIZE-SUB_TRAIN) {
			// get data
			for (int tmp = 0, now = 0; tmp < TRAIN_DATA_SIZE; tmp++) {
				if (tmp < cnt || tmp >= cnt+TRAIN_DATA_SIZE-SUB_TRAIN) {
					for (int j = 0; j < FEATURE_NUM; j++)
						flowers[now].feature[j] = train[tmp].feature[j];
					flowers[now].class = train[tmp].class;
					now++;
				}
			}
			...
			// build decision tree
			depth = 0;
			build_tree(flowers, flowers+SUB_TRAIN-1, root[i]);
		}
		// extra tree : trained by 120 instances
		depth = 0;
		build_tree(train, train+TRAIN_DATA_SIZE, root[ROOT_NUM-1]);
		//**********************************
		//*        End Train Data          *
		//**********************************
		...
		//*      	  Test Data            *
		//**********************************
		for (int cnt = test_data_start; cnt < test_data_start+30; cnt++) {
			int get_class = 0;
			int tmp_get_class[CLASS_NUM] = {0};

			for (int j = 0; j < ROOT_NUM-1; j++) {
				int tmp = test_data(root[j], &data[cnt]);
				tmp_get_class[tmp]++;
			}
			tmp_get_class[test_data(root[ROOT_NUM-1], &data[cnt])] += EX_TREE_WEIGHT;

			for (int j = 0, tmp = 0; j < CLASS_NUM; j++)
				if (tmp_get_class[j] > tmp) get_class = j, tmp = tmp_get_class[j];

			if (get_class == data[cnt].class) {
				accuracy += (double) 1;
